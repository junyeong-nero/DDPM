...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv5 = UNetDown(filters[3], filters[4], is_batchnorm=self.is_batchnorm)
        self.temb5 = UNetTimeEmbedding(time_emb_dim, filters[4])
        self.cemb5 = UNetTimeEmbedding(class_emb_dim, filters[4])
        self.maxpool5 = nn.MaxPool2d(kernel_size=2)

        # bottleneck

        self.center = UNetDown(filters[4], filters[5], is_batchnorm=self.is_batchnorm)
        self.temb_center = UNetTimeEmbedding(time_emb_dim, filters[5])
        self.cemb_center = UNetTimeEmbedding(class_emb_dim, filters[5])

        # upsampling
        self.up_concat5 = UNetUp(filters[5], filters[4], is_deconv=self.is_deconv, is_batchnorm=self.is_batchnorm)
        self.up_temb5 = UNetTimeEmbedding(time_emb_dim, filters[4])
        self.up_cemb5 = UNetTimeEmbedding(class_emb_dim, filters[4])
    ...
    def forward(self, inputs, t, c=None):
        ...
        conv5 = self.conv5(maxpool4)  # [B, 512, 4, 4]
        emb5 = self.temb5(t) + (self.cemb5(c) if c is not None else 0)
        maxpool5 = self.maxpool5(conv5 + emb5)  # [B, 512, 2, 2]

        # bottleneck

        emb_center = self.temb_center(t) + (self.cemb_center(c) if c is not None else 0)
        center = self.center(maxpool5) + emb_center # [B, 1024, 2, 2]

        # upsampling

        up_emb5 = self.up_temb5(t) + (self.up_cemb5(c) if c is not None else 0)
        up5 = self.up_concat5(center, conv5) + up_emb5  # [B, 512, 4, 4]

        up_emb4 = self.up_temb4(t) + (self.up_cemb4(c) if c is not None else 0)
        up4 = self.up_concat4(up5, conv4) + up_emb4  # [B, 512, 4, 4]
